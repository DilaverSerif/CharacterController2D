...
namespace Prime31
{
	...
	public class CharacterController2D : MonoBehaviour
	{
		...
		/// </summary>
		[Tooltip( "When true, one way platforms will be ignored when moving vertically for a single frame" )]
		public bool ignoreOneWayPlatformsThisFrame;

		[SerializeField]
		[Range( 0.001f, 0.3f )]
		[Tooltip( "Defines how far in from the edges of the collider rays are cast from. If cast with a 0 extent it will often result in ray hits that are not desired." )]
		...
		/// </summary>
		[SerializeField]
		[Tooltip( "Mask with all layers that should act as one-way platforms. Note that one-way platforms should always be EdgeCollider2Ds." )]
		...
		[Range( 0f, 90f )]
		[Tooltip( "The max slope angle that the CC2D can climb" )]
		...
		[Range( 2, 20 )]
		[Tooltip( "Total number of horizontal rays used for collision detection" )]
		public int totalHorizontalRays = 8;
		[Range( 2, 20 )]
		[Tooltip( "Total number of vertical rays used for collision detection" )]
		...
		// Used to detect if the collider or scale has changed to recalculate rays
		Vector2 _lastColliderSize;
		...
		void Awake()
		{
			...
			// cache size for dynamic resizing check
			_lastColliderSize = boxCollider.size;
		...
		void primeRaycastOrigins()
		{
			...
			if( boxCollider.size != _lastColliderSize || transform.localScale != _lastLocalScale )
			{
				recalculateDistanceBetweenRays();
				_lastColliderSize = boxCollider.size;
				_lastLocalScale = transform.localScale;
			...
			if( angle < slopeLimit )
			{
				...
				if( deltaMovement.y < jumpingThreshold )
				{
					...
					if( raycastHit )
					{
						...
						deltaMovement = (Vector2)raycastHit.point - (Vector2)ray;
						if( isGoingRight )
							deltaMovement.x -= _skinWidth;
						else
							deltaMovement.x += _skinWidth;
		...
		private void handleVerticalSlope( ref Vector2 deltaMovement )
		{
			...
			if( _raycastHit )
			{
				...
				if( isMovingDownSlope )
				{
					...
					var move = new Vector3( deltaMovement.x * slopeModifier, 0, 0 );
					move = Quaternion.AngleAxis( -angle, Vector3.forward ) * move;
					
					deltaMovement.x = move.x;
					deltaMovement.y += move.y;
